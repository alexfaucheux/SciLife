...
public class Library {
    ...
    String[][] printLibrary(boolean print) {
        ...
        for (int i = 0; i < library.length; i++) {
            for (int k = 0; k < 4; k++) {
                ...
                if (k == 2) {
                    spacing = repeat("Checked In/Out:".length() - library[i][k].length() + 2, " ");
    ...
    boolean Contains(String token, String key, List<Book> books) {
        String bookToken = "";
        books.First();
        for (int i = 0; i < books.GetSize(); i++, books.Next()) {
    ...
    void check(String op, String staff, String owner) {
